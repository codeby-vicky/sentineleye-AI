...
def capture_angle():
    ...
    for _ in range(30): # Give it more attempts to find good frames
        ...
        if frame is not None:
            ...
            if laplacian_var > 100 and 40 < mean_brightness < 250:
                faces = face_detector.detect(frame)
                # Only use frames with exactly one high confidence face
                if faces and len(faces) == 1 and faces[0].confidence > 0.7:
                    frames.append(frame)
                    if len(frames) == 3:
                        break
        time.sleep(0.05)
